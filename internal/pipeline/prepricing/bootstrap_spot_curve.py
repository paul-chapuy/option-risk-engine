...
def fetch_latest_par_yield_points(fred_client: FredClient) -> List[YieldPoint]:
    yield_points = []
...
def get_par_term_structure(api_key: Optional[str] = None) -> YieldTermStructure:
    api_key = api_key or os.environ["FRED_API_KEY"]
    fred_client = FredClient.from_api_key(api_key)
    yield_points = fetch_latest_par_yield_points(fred_client)

    return YieldTermStructure.from_raw_data(
        yield_points,
        term_structure_type=TermStructureType.Par,
        model=NelsonSiegel,
    )
...
def bootstrap_spot_term_structure(
    par_term_structure: YieldTermStructure, max_tenor: float = 30.0, step: float = 0.5
) -> YieldTermStructure:

    spot_points = []
    evaluation_date = par_term_structure.evaluation_date

    dt = step
    while dt <= max_tenor:
        semi_annual_rate = par_term_structure.model(dt)
    ...
    dt_initial = 1 / 12
    r_initial = par_term_structure.points[0].yield_rate
...
def main():
    par_term_structure = get_par_term_structure()
    spot_term_structure = bootstrap_spot_term_structure(par_term_structure)
    ...
    synthetic_fred_par = [par_term_structure.model(dt) for dt in synthetic_tenors]
    synthetic_spot_curve = [spot_term_structure.model(dt) for dt in synthetic_tenors]

    plt.figure(figsize=(8, 4.5))
    plt.scatter(
        par_term_structure.tenors(),
        par_term_structure.yields(),
        label="empirical par",
    )
    plt.plot(synthetic_tenors, synthetic_fred_par, label="NS fit fred par")
